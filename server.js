...
mongoose.connect('mongodb://localhost/domotic-api');

const models = {
  Group: require('./models/groups').model,
  Device: require('./models/devices').model
};

const registryTemplates = {
  groups: require('./models/groups').registry,
  devices: require('./models/devices').registry
};
...
const docs = new API.controllers.Documentation(registry, {name: 'Domotic API'});
